async function apiBuilder(uuid, eventKey) {
    ...
    const EVENT_NAME = "apsak-wallet-message-" + eventKey;
    const EVENT_REPLY = "apsak-wallet-message-reply-" + eventKey;

    const EVENT_WALLET_DISCOVERY = "apsak-wallet-discovery";
    const EVENT_WALLET_ANNOUNCE = "apsak-wallet-announce";

    function log(...args) {
        console.log("📘[apsak-api]:", ...args);
    }
    ...
    // window.apsak = apsak;
    // // // testing ...
    // apsak.signTransaction({tx_info:"xyz"}).then(result=>{
    //     console.log("apsak.signTransaction", result)
    // })
}
